...
namespace HD.Helper.Common
{
    ...
    public class ZipHelper
    {
        ...
        /// 压缩
        /// </summary>
        /// <param name="ListFileToZip">待压缩的文件数组</param>
        /// <param name="ZipedFile">生成的目标文件</param>
        /// <param name="level">6</param>
        /// <returns></returns>
        public static bool EnZip(List<string> ListFileToZip, string ZipedFile, int level)
        {
            bool res = false;
            try
            {

                if (ListFileToZip.Count > 0)
                {

                    ZipOutputStream s = new ZipOutputStream(File.Create(ZipedFile));
                    s.SetLevel(level);
                    res = ZipListFile(ListFileToZip, s, "");
                    s.Finish();
                    s.Close();
        ...
        /// 压缩文件
        /// </summary>
        /// <param name="FileToZip">要进行压缩的文件名 数组</param>
        /// <param name="ZipedFile">压缩后生成的压缩文件名</param>
        /// <param name="level"></param>
        /// <returns></returns>
        private static bool ZipListFile(List<String> ListFileToZip, ZipOutputStream s, string ParentFolderName)
        {
            string FolderToZip = Path.GetDirectoryName(ListFileToZip[0]);
            bool res = true;
            string[] folders, filenames;
            ZipEntry entry = null;
            FileStream fs = null;
            ...
            try
            {
                entry = new ZipEntry(Path.Combine(ParentFolderName, "/"));
                s.PutNextEntry(entry);
                s.Flush();
                //filenames = Directory.GetFiles(FolderToZip);
                foreach (string file in ListFileToZip)
                {
                    fs = File.OpenRead(file);
                    byte[] buffer = new byte[fs.Length];
                    fs.Read(buffer, 0, buffer.Length);
